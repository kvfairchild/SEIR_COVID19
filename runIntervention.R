suppressPackageStartupMessages(if(!require('deSolve')) install.packages('deSolve')); library(deSolve)
suppressPackageStartupMessages(if(!require('plotly')) install.packages('plotly')); library(plotly)
suppressPackageStartupMessages(if(!require('dplyr')) install.packages('dplyr')); library(dplyr)
suppressPackageStartupMessages(if(!require('reshape')) install.packages('reshape')); library(reshape)
suppressPackageStartupMessages(if(!require('htmltools')) install.packages('htmltools')); library(htmltools)
suppressPackageStartupMessages(if(!require('argparse')) install.packages('argparse')); library(argparse)
...
parser$add_argument("--InitInf")
parser$add_argument("--Tint")
parser$add_argument("--Tmax")
parser$add_argument("--varShow")
...
AllowPresym="No"
AllowAsym="Yes"
FracAsym=30 #Fraction of all infections that are asymptomatic
...
N= strtoi(args$N) #Total population size
Tmax= strtoi(args$Tmax) #Maximum time
InitInf= strtoi(args$InitInf) #Initial # infected
yscale="linear"

AllowSeason="No"
seas.amp=0.0 #relative amplitude of seasonal fluctuations, in [0,1]
seas.phase=0 #phase of seasonal fluctuations, measured in days relative to time zero when peak will occur (0=peak occurs at time zero, 30 = peak occurs one month after time zero). Can be negative
#Note that the beta values chosen above will be those for time zero, wherever that occurs in the seasonality of things.

#Intervention parameters
VarShowInt=args$varShow #This is the variable to be plotted. Options "E0", "E1","I0", "I1", "I2", "I3", "R", "D", "Susceptible (S)"="S", "Exposed (E)"="E", "Mild Infections (I1)"="I1", "Severe Infections (I2)"="I2", "Critical Infections (I3)"="I3", "Recovered (R)"="R", "Dead (D)"="D", "All infected (E + all I)"="Inf","All symptomatic (I1+I2+I3)"="Cases","All hospitalized (I2+I3)"="Hosp"),

## TODO: CHECK THESE PARAMS
Tint = strtoi(args$Tint) #Intervention start time (days)
Tend = Tmax #Intervention end time (days)
s0=75 #Reduction in transmission from asymptomatic/presymptomatic infections
s1=70 #Reduction in transmission from mild infections
s2=40 #Reduction in transmission from severe infections
s3=10 #Reduction in transmission rate from critical infections

RoundOne = FALSE #Round values to nearest integer post-intervention?"
...
write.csv(outAll.sub,file=args$saveModel)
